...
export default async function DashboardLayout({
  ...
  if (activeStores.length === 0) {
    console.log("[DASHBOARD_LAYOUT] User has no active stores, redirecting to onboarding", {
      userId: session.user.id,
  ...
  const userRole = activeStores[0]?.role as "ADMIN" | "MANAGER" | "EMPLOYEE" | undefined;

  if (!userRole) {
    console.error("[DASHBOARD_LAYOUT] CRITICAL: Active store exists but no role found");
    redirect("/onboarding")
  }
